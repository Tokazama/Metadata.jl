...
@testset "$f" for f in (sum, prod, maximum, minimum)  # TODO argmax, argmin, extrema
    @test f(mx) == f(x)
    @test f(mx; dims=1) == f(x; dims=1)
    @test metadata(f(mx; dims=1)) == metadata(mx)  # retain metadata
end

@test @inferred(size(mxview, 1)) == @inferred(size(mx, 1))
@test @inferred(axes(mxview, 1)) == @inferred(axes(mx, 1))
